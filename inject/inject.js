document.getElementById('containerPlayerlist').innerHTML += `
    <div class="tooltip-wrapper" style="margin-top:8px">
        <input type="text" placeholder="Image URL" id="imageUrl">
    </div>
    <div class="tooltip-wrapper" style="margin-top:8px">
        <button class="btn btn-warning btn-block" onclick="hacktivate();">Draw URL or upload file</button>
    </div>
    <canvas style="display: none;" width="800" height="600" id="scalingCanvas"></canvas>
`;
...
function closestColor(color){
    ...
    for(colorNumber in palette){
        let currentColorDiff = colorDifference(color, palette[colorNumber]);
        if(currentColorDiff < closestDiff){
            closestDiff = currentColorDiff;
            closest = colorNumber;
        }
    }
    return closest;
...
function drawPart(arr){
    splitArr = chunkArray(arr, 7);
    for(chunk of splitArr){
        for (var e = 0; e < chunk.length; e++) drawI(chunk[e]);
        emitMsg("drawCommands", chunk);
    }
}

function renderColored(image){
    const imageSize = image.height * image.width;
    var outputImage = new Uint8ClampedArray(imageSize)
    for(i = 0; i < imageSize; i++){
        let currentColor = [image.data[i*4], image.data[i*4+1], image.data[i*4+2]];
        let colorMatch = closestColor(currentColor);
        outputImage.set([colorMatch], i);
    }
    return outputImage;
}

function drawFromRendered(imageData, width, height){
    var outputImageData = new Uint8ClampedArray(imageData.length*4);
    for(i = 0; i < imageData.length; i++){
        outputImageData.set([...palette[imageData[i]], 255], i*4);
    }
    return new ImageData(outputImageData, width, height);
}

function skribblDraw(image, width, height){
    var parts = [];
    var lastpart = [];
    for(y = 0; y < height; y++){
        for(x = 0; x < width; x++){
            let color = image[width * y + x];
            let part = [0, color, 1, x, y, x, y];
            if(lastpart[1] == color){
                parts[parts.length-1][5]++;
            }else{
                parts.push(part);
                lastpart = part;
            }
...
function hacktivate(){
    ...
    ctx.fillStyle = 'white';
    ctx.fillRect(0, 0, scalingCanvas.width, scalingCanvas.height);
    
    if(imageUrl.value.length > 0){
        fetchImage = new Image();
        fetchImage.crossOrigin = "anonymous";
        fetchImage.onload = function(){
            ctx.drawImage(fetchImage, 0, 0, 800, 600);
            const imageData = ctx.getImageData(0, 0, 800, 600);
            const renderedImage = renderColored(imageData);
            ctx.putImageData(drawFromRendered(renderedImage, 800, 600), 0, 0);
            skribblDraw(renderedImage, 800, 600);
        }
        fetchImage.onerror = function(){
            alert('Failed to fetch the image, check the console for further info.');
        }
        fetchImage.src = imageUrl.value;
    }else{
